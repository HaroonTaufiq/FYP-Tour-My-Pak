...
var [perk,setperk]=useState('')
  ...
  function add() {
    ...
    fetch("http://localhost:4000/add-guide", {
      method: "post",
      headers: {
  ...
  return (
    ...
    <div className=" mx-auto">
          {preInput('Name', 'Name for your travel guide. should be short and catchy as in advertisement')}
          <input type="text" onChange={(event) => setName((event.target.value.toLowerCase()))} placeholder="name, for example: Liaqat Ali" value={name} />
          {error && !name && <span className='text-red-500 block -ml-[0px]'>Enter valid title </span>}
          {preInput('Address', 'add place for a travel guide')}
    ...
    {error && !img && <span className='text-red-500 block -ml-[0px]'>Upload image</span>}
            ...
            <div className="grid gap-2 grid-cols-2 md:grid-cols-4">
              ...
              <div>
                <h3 className="mt-2 -mb-1">Persons per tour</h3>
              ...
              <div>
                <h3 className="mt-2 -mb-1">Per Day Expense</h3>
